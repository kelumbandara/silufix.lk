    ...
    function drawGooleChart() 
    {
        ...
        $.post('getData_MachineSpeedData.php', { userpara: DataAry }, function(json_data2) 
        {
            ...
            {             
                ...
                dataTable.addColumn({ type: 'datetime', id: 'ServerDatetime' });    // X-axis:
                dataTable.addColumn({ type: 'number', id: 'SD-07' });
                dataTable.addColumn({ type: 'number', id: 'SD-09' });
                dataTable.addColumn({ type: 'number', id: 'SD-02' });
                dataTable.addColumn({ type: 'number', id: 'SD-20' });
                dataTable.addColumn({ type: 'number', id: 'SD-08' });
                               
                if(intDebugEnable === 1) alert("dataTable:" + dataTable);
                dataAry_new.forEach(function(row,index) 
                {  
                     // Replace NULL with JavaScript's null
                    var datetime = row[0]; // ServerDatetime should be non-null
                    var sd07 = (row[1] !== null) ? row[1] : null;
                    var sd09 = (row[2] !== null) ? row[2] : null;
                    var sd02 = (row[3] !== null) ? row[3] : null;
                    var sd20 = (row[4] !== null) ? row[4] : null;
                    var sd08 = (row[5] !== null) ? row[5] : null;

                    dataTable.addRow([datetime, '1', '4', '3', '0', '7']);

                        /*
                    if (intDebugEnable === 1) {
                        alert("row[0]:" + datetime);
                        alert("row[1]:" + sd07);
                        alert("row[2]:" + sd09);
                        alert("row[3]:" + sd02);
                        alert("row[4]:" + sd20);
                ...
                var options = 
                        {
                            title: 'Machine Speeds Over Time', // Add your chart title here
                            height: 440,
               ...
               var options = 
                       {
                            ...
                            hAxis: { title: 'Time' },
                            vAxis: { title: 'Sen1Speed' },
                            height: 500,
                            width: '100%',
                            colors: ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b'], // Define separate colors for each machine
                        };
