...
<?php
    ...
    try 
    {
        ...
        $stmt = $conn->prepare("UPDATE tblwo_event SET WoEventLog=CONCAT(WoEventLog, :evtlog),ReOpenedDateTime=:reopendt,ReOpenedUser=:reopenepf,WoStatus=:wostat,WoVerify=:wovery,State=:stat,WoReOpen=:reopn WHERE WorkOrderNo=:wono");
        $stmt->bindParam(':wono', $strWorkOrderNo);
        $stmt->bindParam(':evtlog', $strAddEventLog);
        $stmt->bindParam(':reopendt', $strServerDateTime);
        $stmt->bindParam(':reopenepf', $strCurrentUserEPF);
