...
<?php
    ...
    if($strFuncType === "funGetCheckInDetails_byWoEpf")      //-------------- funGetCheckInDetails_byWoEpf -----------
    {
        ...
        try 
        {
            ...
            $stmt = $conn->prepare("SELECT ID, CheckInUserDateTime, CheckOutUserDateTime FROM tblwo_allcheckinusers  WHERE WorkOrderNo=:wono AND CheckInUser=:epf AND Status=:stat");
    ...
    else if($strFuncType === "funCheckUserCredentials") //------------- funCheckUserCredentials --------------------
    {
        ...
        {
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);                       
            $stmt = $conn->prepare("SELECT EPF, EmpName, Password, Department,Contact FROM tblusers_account  WHERE UserName=:unme");
