<?php
    ...
    if($strFuncType === "funGetData_CheckInAllocateTimeChart") //------------- funUpdateEventLog --------------------
    {        
        $strEmpEPF         = $num[1]; 
        $strStarDate       = $num[2];
        $strEndDate        = $num[3];        
		...
		$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION); 
            $sqlString = "SELECT
                a.WorkOrderNo,
                a.CheckInUserDateTime,
                a.CheckOutUserDateTime,
                DATE(a.CheckInUserDateTime) AS CheckInUserDate,
                b.MachineNo,
                b.WoDescription
            FROM
                tblwo_allcheckinusers a
                INNER JOIN tblwo_event b ON a.WorkOrderNo = b.WorkOrderNo
            WHERE
                a.CheckInUser = :chkinusr
                AND a.CheckInUserDateTime BETWEEN :start_date AND :end_date               
                AND State > 2
            ORDER BY
                a.CheckInUserDateTime ASC";
	...
	/*
            ...
            WHERE
                a.CheckInUser = :chkinusr
                AND a.CheckInUserDateTime BETWEEN :start_date AND :end_date
                AND TIME(a.CheckInUserDateTime) BETWEEN '07:00' AND '19:00'
                AND State > 2
            ORDER BY
                a.CheckInUserDateTime ASC";
	*/
            ...
            {   
                $ReturnData_ary[$i][0] = $row['CheckInUserDate'];   //"CheckIn Time";
