    ...
    try 
    {
       ...
       $stmt = $conn->prepare("
            SELECT 
                DATE_FORMAT(CreatedDateTime, '%m-%d') AS DatePart, 
                COUNT(WorkOrderNo) AS TotPlacedWorkOrders,
                SUM(CASE WHEN State > 2 AND ClosedDateTime >= DATE_SUB(NOW(), INTERVAL 30 DAY) AND DATE_FORMAT(ClosedDateTime, '%m-%d') = DATE_FORMAT(CreatedDateTime, '%m-%d') THEN 1 ELSE 0 END) AS CompletedWorkOrders,
                SUM(CASE WHEN WorkOrderCategory='Breakdown' THEN 1 ELSE 0 END) AS TotPlacedBreakDown,
                SUM(CASE WHEN WorkOrderCategory='Breakdown' AND State > 2 AND ClosedDateTime >= DATE_SUB(NOW(), INTERVAL 30 DAY) AND DATE_FORMAT(ClosedDateTime, '%m-%d') = DATE_FORMAT(CreatedDateTime, '%m-%d') THEN 1 ELSE 0 END) AS CompletedBreakDown, 
                SUM(CASE WHEN WorkOrderCategory='Breakdown' AND State > 2 AND ClosedDateTime >= DATE_SUB(NOW(), INTERVAL 30 DAY) AND DATE_FORMAT(ClosedDateTime, '%m-%d') = DATE_FORMAT(CreatedDateTime, '%m-%d') THEN TIMESTAMPDIFF(MINUTE, CreatedDateTime, ClosedDateTime) ELSE 0 END) AS TotalBreakDownDuration
            FROM 
                tblwo_event
            WHERE 
                CreatedDateTime >= DATE_SUB(NOW(), INTERVAL 30 DAY)
