        ...
        <div class="content-wrapper">
            ...
            <section class="content">
                <div class="container-fluid">
                                ...
                                <table>
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td style="width:5%;"></td>   
                                            <td style="width:25%;" id="id_Home_TotBreakDownDuration_1day">: - Min</td>   
                                        </tr> 
                                        <tr>
                    ...
                    <!-- getCount_Redtag_breakdown_services -->
                    <div class="border-top my-1"></div>
                    <div class="row"> 
                        <!-- current checking work order -->
                      <div class="col-lg-5">
                            <p class="text-center"><strong>current checking work order</strong></p>                        
                        </div>
                        
                        <div class="col-lg-3">
                            <input type="text" id="myCustomSearchBox2" class="form-control" placeholder="Search Anything here">
                        </div>
                    </div>
                    <div class="border-top my-1"></div>
                    <div class="row">
                        ...
                        <section class="col-lg-12 connectedSortable">
                            <!-- Custom tabs (Charts with tabs)-->
                           <table id="example2" class="table table-bordered table-striped display compact">
                                <thead class="bg-info">
                                    <tr>
                                        <th>#</th>
                                        <th>WO</th>
...
<script>   
    ...
    {     
        ...
        getCount_Redtag_breakdown_services();
        funRefresh_current_checking_work_order();
    ...
    $('#myCustomSearchBox').keyup(function() 
    {
     ...
     $('#myCustomSearchBox2').keyup(function() 
    {
        dtbl5.search($(this).val()).draw(); // this  is for customized searchbox with datatable search feature.
    });
    ...
    function funRefresh_Last30DaySummary() 
    {
        ...
        $.post('class/getData_HomeSummary.php', { userpara: DataAry }, function(json_data2) 
        {
            ...
            if(res.Status_Ary[0] === "true")
            {
                ...
                document.getElementById("id_Home_TotBreakDownDuration_1day").innerHTML = " :   " + convertMinutesToTime(res.Data_Ary[1]); 
    ...
    function functionDailyPieChart(){
        ...
        $.post('class/getData_EventTable.php', { userpara: DataAry }, function(json_data2) 
        {
            if(intDebugEnable === 1)    alert(json_data2);           
            var res = $.parseJSON(json_data2); 
            if(intDebugEnable === 1)    alert(res.Status_Ary[0]); 
            
            var RedTags_count        = res.Count_ary_RedTag;
            var RedTags_IssueTypes   = res.IssueType_ary_RedTag;
            var BreakDown_count      = res.Count_ary_Breakdown;
            var BreakDown_IssueTypes = res.IssueType_ary_Breakdown;
        
            RenderPieChart("Breakdown",BreakDown_IssueTypes,BreakDown_count);
            RenderPieChart("RedTags",RedTags_IssueTypes,RedTags_count);
...
function RenderPieChart(chart, labels, count) {
    ...
    if (chart === "RedTags") {
        chartID = "red_tag_canvas";
    } else if (chart === "Breakdown") {
        chartID = "Breakdown_tag_canvas";
    }
    ...
    function getCount_Redtag_breakdown_services()
    {
        let intDebugEnable = 0;        
        if(intDebugEnable === 1)    alert("getCount_Redtag_breakdown_services");
        const DataAry = [];  
        DataAry[0] = "getCount_Redtag_breakdown_services";        
        if(intDebugEnable === 1)    alert(DataAry);
        $.post('class/getCount_Redtag_breakdown_services.php', { userpara: DataAry }, function(json_data2) 
        {
            if(intDebugEnable === 1)    alert(json_data2);           
            var res = $.parseJSON(json_data2); 
            if(intDebugEnable === 1)    alert(res.Status_Ary[0]); 
            var RedTags_count    = res.RedtagCount;
            var BreakDown_count  = res.BreakDown_count;
            var Service_count    = res.Service_count;
            ...
            document.getElementById('id_No_of_Redtag').innerHTML    = RedTags_count ;
            document.getElementById('id_No_of_Breakdown').innerHTML = BreakDown_count;
            document.getElementById('id_No_of_Services').innerHTML  = Service_count ;
            
        });
    }

    //current checking work order
    function funRefresh_current_checking_work_order() 
    {
        let intDebugEnable = 0;        
        if(intDebugEnable === 1)    alert("funRefresh_current_checking_work_order");
        ...
        if(intDebugEnable === 1)    alert("DataAry : " + DataAry);
        //-------------- Update Home page WO Table ---------------------------------------------
        $.post('class/getData_current_checking_work_order.php', { userpara: DataAry }, function(json_data2) 
        {
            if(intDebugEnable === 1)    alert("json_data2 : " + json_data2); 
            var res = $.parseJSON(json_data2); 
            //alert(json_data2) ;
            ...
            if(res.Status_Ary[0] === "true")
            {
                ...
                {       
                    ...
                    dtbl5.row.add([
                        intX, 
                        res.Data_Ary[i][1],
                        res.Data_Ary[i][2].substring(2, 16), 
                        res.Data_Ary[i][3],  
                        res.Data_Ary[i][4],//user
                        res.Data_Ary[i][5], //
                        res.Data_Ary[i][6],
                        res.Data_Ary[i][7], 
                        res.Data_Ary[i][8]
