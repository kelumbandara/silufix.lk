...
<?php
    ...
    if($strFuncType === "funGetCheckInDetails_byWoEpf")      //-------------- funGetCheckInDetails_byWoEpf -----------
    {
        ...
        try 
        {
            ...
            $stmt = $conn_mc->prepare("SELECT ID, CheckInUserDateTime, CheckOutUserDateTime FROM tblwo_allcheckinusers  WHERE WorkOrderNo=:wono AND CheckInUser=:epf AND Status=:stat");
    ...
    else if($strFuncType === "funUpdateEventLog") //------------- funUpdateEventLog --------------------
    {
        ...
        {
            $conn_mc->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);   
            //    $stmt = $conn_mc->prepare("UPDATE tblwo_event SET WoEventLog=CONCAT(WoEventLog, :evtlog),WoVerify=:wovery,State=:stat WHERE WorkOrderNo=:wono");
            //$stmt = $conn_mc->prepare("UPDATE tblwo_event SET WoEventLog=CONCAT(WoEventLog, :evtlog) WHERE WorkOrderNo=:wono");
            $stmt = $conn_mc->prepare("UPDATE tblwo_event SET WoEventLog=CONCAT(WoEventLog, :evtlg) WHERE WorkOrderNo=:wono");
    ...
    else if($strFuncType === "funCheckUserCredentials") //------------- funCheckUserCredentials --------------------
    {
        ...
        {
            $conn_mc->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);                       
            $stmt = $conn_mc->prepare("SELECT EPF, EmpName, Password, Department,Contact FROM tblusers_account  WHERE UserName=:unme");
            ...
            {                            
                ...
                $ReturnData_ary[2]      = $row['Contact'];
                $ReturnData_ary[3]      = $row['Department'];
    ...
    else if($strFuncType === "funGetUserDetails_byEPF") //------------- funCheckUserCredentials --------------------
    {
        ...
        {
            $conn_mc->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);                       
            $stmt = $conn_mc->prepare("SELECT ID,EPF,EmpName,UserName,Department,Contact,Email,UserType,Availability,Status FROM tblusers_account  WHERE EPF=:uepf");
    ...
    else if($strFuncType === "funGetCheckInUserData") //------------- funGetCheckInUserData --------------------
    {
        ...
        $strSQL =   "SELECT DISTINCT 
                        ...
                        TIMEDIFF(NOW(), tblwo_allcheckinusers.CheckInUserDateTime) AS CheckInDuration
                    FROM
    ...
    else if($strFuncType === "funGetCheckInUserData_All") //------------- funGetCheckInUserData --------------------
    {
        $strWorkOrderNo   = $num[1];  
        ...
        $strSQL =   "SELECT DISTINCT 
                        ...
                        tblusers_account.Contact,
                        tblusers_account.EmpName,
                        tblwo_allcheckinusers.CheckInUserDateTime,
                        tblwo_allcheckinusers.CheckOutUserDateTime,
                        TIMEDIFF(tblwo_allcheckinusers.CheckOutUserDateTime, tblwo_allcheckinusers.CheckInUserDateTime) AS CheckInDuration
                    FROM
                        tblwo_allcheckinusers
                    JOIN
                        tblusers_account ON tblwo_allcheckinusers.CheckInUser = tblusers_account.EPF
                    WHERE
                        tblwo_allcheckinusers.WorkOrderNo = :wono AND tblwo_allcheckinusers.Status ='Deactive'";
    ...
    else if($strFuncType === "funGetAllocatedUserData") //------------- funGetCheckInUserData --------------------
    {
        ...
        $strSQL =   "SELECT DISTINCT 
                        ...
                        tblwo_allocatedusers.AllocatedUserStartDateTime,
                        tblwo_allocatedusers.AllocatedUserEndDateTime,
                        tblusers_account.Contact,
                        tblusers_account.EmpName,
                        TIMESTAMPDIFF(MINUTE, tblwo_allocatedusers.AllocatedUserStartDateTime, tblwo_allocatedusers.AllocatedUserEndDateTime) AS DurationInMinutes
                    FROM
    ...
    else if($strFuncType === "funGetChatHistoryData") //------------- funGetChatHistoryData --------------------
    {
        ...
        $strSQL =   "SELECT DISTINCT 
                        ...
                        tblusers_account.Contact,
                        tblusers_account.EmpName,
                        tblwo_chathistory.ID
                    FROM
                        tblwo_chathistory
                    JOIN
                        tblusers_account ON tblwo_chathistory.SendBy = tblusers_account.EPF
                    WHERE
                        tblwo_chathistory.WorkOrderNo = :wono AND tblwo_chathistory.Status= 'Active' ORDER BY tblwo_chathistory.ID DESC";
