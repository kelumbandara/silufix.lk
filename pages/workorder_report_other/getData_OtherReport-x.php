<?php
    ...
    if($strFuncType === "funGetData_PieChart1") //------------- funUpdateEventLog --------------------
    {
        ...
        $whereClause = "WorkOrderCategory = 'OtherProject' AND State < 6 AND ClosedDateTime IS NOT NULL AND CreatedDateTime BETWEEN :sdate AND :edate";
        ...
        {   
            $sqlString = "
                SELECT 
                    COUNT(CASE WHEN WorkOrderSubCategory = 'Repair' THEN 1 END) AS Repair_Count,
                    COUNT(CASE WHEN WorkOrderSubCategory = 'Maintenance' THEN 1 END) AS Maintenance_Count,
                    COUNT(CASE WHEN WorkOrderSubCategory = 'Machine Spare Part Order' THEN 1 END) AS MchSpPartOrder_Count,
                    COUNT(CASE WHEN WorkOrderSubCategory = 'Service' THEN 1 END) AS Service_Count,
            ...
            {   
                $ReturnData_ary[0][0] = $row['Repair_Count'];
                $ReturnData_ary[0][1] = $row['Maintenance_Count']; 
                $ReturnData_ary[0][2] = $row['MchSpPartOrder_Count']; 
                $ReturnData_ary[0][3] = $row['Service_Count'];
    ...
    else if($strFuncType === "funGetData_Table") 
    {
        ...
        $whereClause = "wo.WorkOrderCategory = 'OtherProject' AND State < 6 AND wo.ClosedDateTime IS NOT NULL AND wo.CreatedDateTime BETWEEN :sdate AND :edate";
